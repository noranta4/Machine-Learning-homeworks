...
if kfold_check != 0:
    ...
    len_training_data = len(X_train)
    pieces_x = list(np.split(X_train, kfold_parts))
    pieces_y = list(np.split(y_train, kfold_parts))
...
for gamma in gamma_list:
    for c in C_list:
        for iteration in range(kfold_parts):
            working_pieces_x = list(pieces_x)
            working_pieces_y = list(pieces_y)
            X_validation = working_pieces_x.pop(iteration)
            y_validation = working_pieces_y.pop(iteration)
            X_train = np.concatenate(tuple(working_pieces_x))
            y_train = np.concatenate(tuple(working_pieces_y))
            clf = SVC(C=c, kernel=kernel, gamma=gamma)
            clf.fit(X_train, y_train)
            score = clf.score(X_validation, y_validation)
            temp_score_list.append(score)
        score = np.average(temp_score_list)
        temp_score_list = []
